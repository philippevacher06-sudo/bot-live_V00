...
def execute_from_cycle(headers, assets):
    ...
    print()
    print("=" * 160)
    print("06G — EXECUTION DEPUIS CYCLE_STATE — DRY_RUN")
    print("=" * 160)
    ...
    for asset in assets:
        ...
        if not cycle:
            if active_exec:
                ...
                if SEND_REAL_ORDERS:
                    exec_state["closed"].append(active_exec)
                    exec_state["active"].pop(asset, None)
        ...
        if active_exec:
            ...
            if not SEND_REAL_ORDERS:
                # En DRY_RUN, on montre la fermeture théorique mais on ne marque rien.
                # On continue pour afficher aussi l'ouverture théorique du nouveau niveau.
                pass
            else:
                exec_state["closed"].append(active_exec)
                exec_state["active"].pop(asset, None)
        ...
        # 7. En réel seulement : confirmer et stocker le dealId
        if SEND_REAL_ORDERS:
