...
# ============================================================
# V24.1 — Clamp scalp Bollinger L1
# ============================================================
# Objectif :
# - conserver Bollinger comme ancre logique
# - empêcher L1 d'être placé trop loin du prix actuel
# - utiliser BOLLINGER_CURSOR comme curseur réel de scalping
#
...
def v241_apply_bollinger_scalp_clamp(asset, direction, levels, cycle_id=None):
    ...
    if not out.get("L1") or not out.get("L2") or not out.get("L3"):
        return out
    ...
    raw_l1 = float(out["L1"])
    step = float(out.get("step") or abs(float(out["L2"]) - float(out["L1"])))
    decimals = v241_level_decimals(asset)

    raw_bid, raw_ask = v24_market_bid_ask(asset)
    low_side = min(float(raw_bid), float(raw_ask))
    high_side = max(float(raw_bid), float(raw_ask))

    cursor = float(v241_bollinger_cursor())
    applied = False

    if direction == "BUY":
        reference = low_side
        raw_dist = reference - raw_l1

        # Si L1 Bollinger est trop loin sous le prix,
        # on le rapproche à distance scalp.
        if raw_dist > max_dist:
            final_l1 = reference - max_dist
            applied = True
        else:
            final_l1 = raw_l1

        final_l2 = final_l1 - step
        final_l3 = final_l1 - 2.0 * step
        final_dist = reference - final_l1

    else:
        reference = high_side
        raw_dist = raw_l1 - reference

        # Si L1 Bollinger est trop loin au-dessus du prix,
        # on le rapproche à distance scalp.
        if raw_dist > max_dist:
            final_l1 = reference + max_dist
            applied = True
        ...
        final_l2 = final_l1 + step
        final_l3 = final_l1 + 2.0 * step
        final_dist = final_l1 - reference

    out["raw_L1_before_scalp_clamp"] = round(raw_l1, decimals)
    out["scalp_clamp_applied"] = bool(applied)
    out["scalp_reference_price"] = round(reference, decimals)
    out["scalp_raw_distance"] = float(raw_dist)
    out["scalp_final_distance"] = float(final_dist)
    out["scalp_max_distance"] = float(max_dist)
    out["scalp_cursor"] = float(cursor)

    out["L1"] = round(final_l1, decimals)
    out["L2"] = round(final_l2, decimals)
    out["L3"] = round(final_l3, decimals)

    if applied:
        v24_event_safe({
            "event": "V24_BOLLINGER_SCALP_CLAMP",
            "asset": asset,
            "cycle_id": cycle_id,
            "direction": direction,
            "raw_L1": round(raw_l1, decimals),
            "final_L1": out["L1"],
        ...
        print(
            f"{asset:10s} | BOLLINGER_SCALP_CLAMP | "
            f"{direction:4s} | "
            f"raw_L1={round(raw_l1, decimals)} | "
            f"final_L1={out['L1']} | "
            f"raw_dist={raw_dist:.5f} | "
            f"max_dist={max_dist:.5f} | "
            f"cursor={cursor:.2f}"
        )
...
def v24_enrich_cycle_with_bollinger_m5(headers, cycle):
    ...
    # V24.1 — Clamp scalp :
    # si L1 Bollinger est trop éloigné du prix actuel,
    # on le rapproche selon la distance max par actif pilotée par BOLLINGER_CURSOR.
    levels = v241_apply_bollinger_scalp_clamp(
        asset=asset,
        direction=direction,
        levels=levels,
        cycle_id=cycle.get("cycle_id"),
...
def place_limit_order_secure(headers, asset, cycle, market_status, basket_level=None):
    ...
    try:
        ...
        if str(e).startswith("BOLLINGER_WIDTH_TOO_SMALL"):
            ...
            print(
                f"{_asset:10s} | BOLLINGER_WIDTH_TOO_SMALL_SKIP | -- | {_direction:4s} | "
                f"--         | TRADEABLE | {e}"
            )
...
def v24_open_basket_limits_secure(headers, asset, cycle, market_status):
    ...
    if rejects:
        ...
        orphan_close_results = v24_close_orphan_positions_after_partial_reject(
            headers=headers,
            asset=asset,
            reason="V24_BASKET_PARTIAL_REJECT_ORPHAN_CLOSE",
            levels=levels,
        )
...
def v24sa_guard_stop(payload):
    ...
    disable_gs = v24sa_env_csv_set(
        ...
        "EURJPY,USDJPY,OIL_CRUDE,OIL_BRENT,BTCUSD,ETHUSD,J225,SILVER"
    )
...
def v24_process_basket(headers, asset, cycle, active_exec, broker_items, broker_ids, positions, market_status, exec_state):
    ...
    if open_count > 0 and pending_count > 0:
        ...
        try:
            partial_max_age = max(
                10.0,
                float(os.getenv("PARTIAL_BASKET_PENDING_MAX_AGE_SEC", "90"))
            )
        except Exception:
            partial_max_age = 90.0
