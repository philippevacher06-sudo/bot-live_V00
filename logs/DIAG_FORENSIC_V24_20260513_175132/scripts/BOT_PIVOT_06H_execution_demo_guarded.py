...
def count_real_exec_deals():
    """
    V24 Basket-aware :
    On lit le fichier EXEC utilisé par 06G2 et on considère qu'un panier V24 actif
    nécessite une passe broker, même si le moteur STATE est revenu IDLE.
    """
    exec_file = Path(G.B.EXEC_STATE_FILE)
    ...
    for asset, deal in active.items():
        ...
        # V24 : panier actif avec niveaux L1/L2/L3.
        if deal.get("basket_mode") == "V24_BASKET_LIMIT":
            levels = deal.get("levels", {}) if isinstance(deal.get("levels", {}), dict) else {}

            if levels:
                for level_key, leg in levels.items():
                    if not isinstance(leg, dict):
                        continue
                    if leg.get("dry_run") is True:
                        continue

                    real_deals.append({
                        "asset": asset,
                        "dealId": leg.get("brokerDealId") or leg.get("dealId"),
                        "workingDealId": leg.get("workingDealId"),
                        "level": leg.get("level") or level_key,
                        "direction": leg.get("direction") or deal.get("direction"),
                        "size": leg.get("size"),
