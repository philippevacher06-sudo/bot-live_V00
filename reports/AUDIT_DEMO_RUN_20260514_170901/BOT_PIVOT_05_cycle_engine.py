...
def v242_signal_bollinger_width_ok(asset, sig):
    """
    Early BB-width gate when the signal producer already provides M5 bands.
    If the fields are absent, 06G2 remains the authoritative pre-order filter.
    """
    import os

    width = val(sig, ["bb_width_m5", "BB_WIDTH", "bollinger_width_m5"], None)
    low = val(sig, ["bb_lower_m5", "BB_LOW", "bollinger_lower_m5"], None)
    high = val(sig, ["bb_upper_m5", "BB_HIGH", "bollinger_upper_m5"], None)

    try:
        if width is None and low is not None and high is not None:
            width = abs(float(high) - float(low))
        elif width is not None:
            width = float(width)
        else:
            return True, "NO_SIGNAL_BB_WIDTH", None, None
    except Exception:
        return True, "INVALID_SIGNAL_BB_WIDTH_IGNORED", None, None

    asset = str(asset).upper()
    required_raw = os.environ.get(f"V242_MIN_BB_WIDTH_{asset}") or os.environ.get("V242_MIN_BB_WIDTH")
    try:
        required = float(required_raw) if required_raw not in (None, "") else float(V242_MIN_BB_WIDTH_BY_ASSET.get(asset, 0.0))
    except Exception:
        required = float(V242_MIN_BB_WIDTH_BY_ASSET.get(asset, 0.0))

    if required > 0 and width < required:
        return False, "BOLLINGER_WIDTH_TOO_SMALL_EARLY", width, required

    return True, "BOLLINGER_WIDTH_OK_EARLY", width, required
    ...
    for asset in assets:
        ...
        if status != "IN_POSITION" or not cycle:
            if decision in ("BUY", "SELL"):
                ...
                bb_ok, bb_reason, bb_width, bb_required = v242_signal_bollinger_width_ok(asset, sig)
                if not bb_ok:
                    print(
                        f"{asset:10s} | IDLE_BB_WIDTH | "
                        f"signal {decision} ignore | "
                        f"width={bb_width:.5f} required={bb_required:.5f}"
                    )
                    set_idle(state, asset, bb_reason)
                    continue
                ...
                # V24.3.5I_DEMO_SIGNAL_SAMPLER:
                # Mode demonstration: si aucun signal strict, creer un signal faible
                # depuis la position de range pour generer des exemples reels DEMO.
                demo_sampler = str(__import__("os").getenv("V2435I_DEMO_SIGNAL_SAMPLER", "0")).lower() in (
                    "1", "true", "yes", "y", "on"
                )
                ...
                if demo_sampler and isinstance(sig, dict):
                    ...
                    rpos = _first_float(("range_position", "range_pos", "rpos", "R.Pos", "r_pos"))
                    spread_ok = _first_bool(("spread_ok", "spr_ok", "SprOK"))
                    if not spread_ok and "spread_ok" not in sig and "spr_ok" not in sig and "SprOK" not in sig:
                        spread_ok = True

                    low_thr = float(__import__("os").getenv("V2435I_DEMO_RPOS_LOW", "25"))
                    high_thr = float(__import__("os").getenv("V2435I_DEMO_RPOS_HIGH", "75"))

                    if spread_ok and rpos is not None and rpos >= high_thr:
                        sample_decision = "SELL"
                        sample_reason = f"rpos={rpos:.1f}>={high_thr:.1f}"
                    elif spread_ok and rpos is not None and rpos <= low_thr:
                        sample_decision = "BUY"
                        sample_reason = f"rpos={rpos:.1f}<={low_thr:.1f}"

                if sample_decision:
                    entry_price = real_entry_price(sample_decision, sig)
                    if entry_price is not None:
                        c = new_cycle(asset, sample_decision, entry_price, level=1)
                        c["demo_sample_signal"] = True
                        c["demo_sample_reason"] = sample_reason
                        c["demo_sample_version"] = "V24.3.5I_DEMO_SIGNAL_SAMPLER"
                        set_position(state, asset, c, "OPEN_L1")
                        print(
                            f"{asset:10s} | DEMO_SAMPLE_SIGNAL | "
                            f"{sample_decision:4s} | reason={sample_reason}"
                        )
                        print(
                            f"{asset:10s} | OPEN_L1      | "
                            f"{sample_decision:4s} | "
                            f"entry={entry_price:.6f} | "
                            f"size={float(c['size']):.6f} | "
                            f"tp={float(c['tp_eur']):.4f}"
                        )
                    else:
                        print(f"{asset:10s} | IDLE_DEMO_SAMPLE_NO_PRICE | aucun signal sample | prix_entree_indisponible")
                        set_idle(state, asset, "IDLE_DEMO_SAMPLE_NO_PRICE")
                else:
