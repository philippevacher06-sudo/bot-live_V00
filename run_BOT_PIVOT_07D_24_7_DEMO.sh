...
V2437_MAX_ACTIVE_CYCLES="${V2437_MAX_ACTIVE_CYCLES:-96}"
export V2437_MAX_ACTIVE_CYCLES
...
try:
    ...
    flatten_results = []
    if str(os.environ.get("V2437_EXIT_FLATTEN_POSITIONS_ON_CLEANUP", "0")).lower() not in ("0", "false", "no", "off"):
        ps_flat, pos_flat, raw_flat = B.broker_positions(headers)
        print(json.dumps({
            "event": "V2437_EXIT_POSITION_FLATTEN_BEFORE",
            "positions_status": ps_flat,
            "positions_count": len(pos_flat),
        }, ensure_ascii=False))
        for item in pos_flat:
            asset_flat = B.broker_position_epic(item)
            deal_id = B.broker_position_deal_id(item)
            direction_flat = ((item.get("position") or {}).get("direction") or "")
            size_flat = ((item.get("position") or {}).get("size") or None)
            info = {
                "asset": asset_flat,
                "dealId": deal_id,
            ...
            print(json.dumps({"event": "V2437_EXIT_POSITION_FLATTEN_REQUEST", **info}, ensure_ascii=False))
            if not deal_id:
                info.update({"ok": False, "reason": "MISSING_DEAL_ID"})
                flatten_results.append(info)
                print(json.dumps({"event": "V2437_EXIT_POSITION_FLATTEN_RESULT", **info}, ensure_ascii=False))
                continue
            try:
                st_close, data_close = B.api_delete(headers, f"/api/v1/positions/{deal_id}")
                ok_close = st_close in (200, 201, 202)
                info.update({
                    "ok": ok_close,
                    "delete_status": st_close,
                    "delete_response": data_close,
                })
            except Exception as flat_exc:
                info.update({"ok": False, "exception": str(flat_exc)})
            flatten_results.append(info)
            print(json.dumps({"event": "V2437_EXIT_POSITION_FLATTEN_RESULT", **info}, ensure_ascii=False))
    ...
    print(json.dumps({
        ...
        "flatten_results": flatten_results if 'flatten_results' in locals() else [],
    }, ensure_ascii=False))
...
PY
  ...
  if [ "${V2437_RESET_LOCAL_AFTER_SUCCESSFUL_CLEANUP:-0}" = "1" ]; then
    echo "" | tee -a "$LOG"
    echo "V2437_RESET_LOCAL_AFTER_CLEANUP — $(date -u '+%Y-%m-%dT%H:%M:%SZ')" | tee -a "$LOG"
    python - <<'PYRESET' 2>&1 | tee -a "$LOG"
import json
import subprocess
import BOT_PIVOT_06G_execution_from_cycle_state as B
import BOT_PIVOT_06G2_execution_secure as X
...
try:
    ...
    ps, positions, _ = B.broker_positions(headers)
    ws, wraw = B.api_get(headers, "/api/v1/workingorders")
    pending = X.normalize_working_orders_response(wraw)
    ok_flat = (ps == 200 and ws == 200 and len(positions) == 0 and len(pending) == 0)
    print(json.dumps({
        "event": "V2437_RESET_LOCAL_AFTER_CLEANUP_CHECK",
        "positions_status": ps,
        "positions_count": len(positions),
        "workingorders_status": ws,
        "workingorders_count": len(pending),
        "will_reset_local": ok_flat,
    }, ensure_ascii=False))
    if ok_flat:
        r = subprocess.run(
            ["python", "BOT_PIVOT_99_FORCE_RESET_ALL_LIVE_LOCAL.py"],
            text=True,
            capture_output=True,
            timeout=60,
        )
        print(json.dumps({
            "event": "V2437_RESET_LOCAL_AFTER_CLEANUP_RESULT",
        ...
        print(json.dumps({
            "event": "V2437_RESET_LOCAL_AFTER_CLEANUP_SKIPPED",
            "reason": "BROKER_NOT_FLAT",
        }, ensure_ascii=False))
except Exception as exc:
    print(json.dumps({
        "event": "V2437_RESET_LOCAL_AFTER_CLEANUP_ERROR",
        "error": str(exc),
    }, ensure_ascii=False))
PYRESET

    echo "" | tee -a "$LOG"
    echo "V2437_RECONCILE_AFTER_LOCAL_RESET — $(date -u '+%Y-%m-%dT%H:%M:%SZ')" | tee -a "$LOG"
    python BOT_PIVOT_06G1_reconcile_broker.py 2>&1 | tee -a "$LOG" || true
  fi
}
...
while true; do
  ...
  echo "V2437_EXEC_GUARD_AUDIT max_loop_cycles=$MAX_CYCLES max_active_cycles=$V2437_MAX_ACTIVE_CYCLES" | tee -a "$LOG"
  python BOT_PIVOT_06H_execution_demo_guarded.py --real-demo --unlock OK_ENVOI_DEMO --max-cycles "$V2437_MAX_ACTIVE_CYCLES" --assets "$ASSETS" 2>&1 | tee -a "$LOG"
