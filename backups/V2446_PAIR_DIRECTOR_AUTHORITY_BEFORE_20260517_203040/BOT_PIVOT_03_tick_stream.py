...
def create_session() -> Dict[str, Any]:
    # load_dotenv_files() # Neutralisé pour forcer l'environnement actif
...
class TickCollector:
    ...
    def on_quote(self, payload: Dict[str, Any]) -> None:
        ...
        # Injection RAM Flash V2447 Hyper-Vitesse
        try:
            import json, time
...
async def run_stream(assets: List[str], seconds: int, print_every: int) -> None:
    ...
    recv_timeout_sec = max(1.0, float(os.environ.get("TICK_STREAM_RECV_TIMEOUT_SEC", "30")))
    ...
    async with websockets.connect(stream_url, ping_interval=None, close_timeout=10) as ws:
        ...
        try:
            while True:
                ...
                if seconds > 0:
                    timeout = min(timeout, max(1.0, float(seconds) - elapsed))
                ...
                try:
                    ...
                    if seconds > 0 and time.time() - start >= seconds:
                        log.warning(
                            "[STREAM_TIMEOUT] aucun message WebSocket avant la fin "
                            "de la fenêtre de collecte ; sortie propre sans traceback"
                        )
                        break

                    log.warning(
                        f"[STREAM_TIMEOUT] aucun message WebSocket depuis {timeout:.1f}s ; "
                        "reconnexion au prochain cycle 07D"
                    )
                    break
