...
# V24.4 — audit expérimental fermeture par paire faible/forte
try:
...
def v24_validate_bollinger_limit_levels(asset, direction, levels, price_audit=None, cycle_id=None):
    ...
    try:
        ...
        use_stream_for_bb_distance = str(__import__("os").getenv("V2438_USE_STREAM_PRICE_FOR_BOLLINGER_DISTANCE", "1")).lower() in (
            "1", "true", "yes", "y", "on"
        )

        price_source = "UNKNOWN"
        if use_stream_for_bb_distance and stream_bid is not None and stream_ask is not None:
            raw_bid, raw_ask = stream_bid, stream_ask
            price_source = "STREAM_FOR_BOLLINGER_DISTANCE"
        elif use_stream_guard and stream_bid is not None and stream_ask is not None:
            raw_bid, raw_ask = stream_bid, stream_ask
            price_source = "STREAM_GUARD"
            ...
            raw_bid, raw_ask = v24_market_bid_ask(asset)
            price_source = "MARKET_FALLBACK"

        try:
            print(
                f"{asset:10s} | V2438_BB_PRICE_SOURCE_AUDIT | {direction:4s} | "
                f"source={price_source} stream_bid={stream_bid} stream_ask={stream_ask} "
                f"snapshot_bid={snapshot_bid} snapshot_ask={snapshot_ask} "
                f"chosen_bid={raw_bid} chosen_ask={raw_ask}"
            )
            v24_event_safe({
                "event": "V2438_BB_PRICE_SOURCE_AUDIT",
                "asset": asset,
                "direction": direction,
                "price_source": price_source,
                "stream_bid": stream_bid,
    ...
    try:
        ...
        if _v2433_enabled and _l1 is not None and _l2 is not None and _dist is not None:
            ...
            if _stepf > 0:
                ...
                try:
                    _v2437_audit = str(_v2433_os.getenv("V2437_AUDIT_EVERY_DECISION", "1")).strip().lower() not in ("0", "false", "no", "off")
                    if _ratio <= 3.0:
                        _risk_band = "ALLOWED_NORMAL"
                    elif _ratio <= 5.0:
                        _risk_band = "ALLOWED_AGGRESSIVE"
                    elif _ratio <= _v2433_mult:
                        _risk_band = "ALLOWED_EXTREME_DEMO"
                    else:
                        _risk_band = "BLOCKED_TOO_FAR"

                    if _v2437_audit:
                        print(
                            f"{asset:10s} | V2437_BB_DISTANCE_AUDIT | "
                            f"1-3 | {direction:4s} | --         | AUDIT     | "
                            f"risk_band={_risk_band} "
                            f"L1={_l1f} L2={_l2f} "
                            f"dist={_distf:.10f} step={_stepf:.10f} "
                            f"max_allowed={_allowed:.10f} "
                            f"dist_step_ratio={_ratio:.3f} "
                            f"threshold_mult={_v2433_mult:.3f} "
                            f"old_max_dist={_max_dist_old}"
                        )
                        v24_event_safe({
                            "event": "V2437_BB_DISTANCE_AUDIT",
                            "asset": asset,
                            "cycle_id": cycle.get("cycle_id"),
                            "direction": direction,
                            ...
                            "threshold_mult": _v2433_mult,
                            "old_max_bollinger_distance": _max_dist_old,
                            "decision_will_block": bool(_distf > _allowed),
                        })
                except Exception as _v2437_e:
                    print(
                        f"{asset:10s} | V2437_BB_DISTANCE_AUDIT_ERROR | "
                        f"1-3 | {direction:4s} | --         | AUDIT     | err={_v2437_e}"
                    )
...
def place_limit_order_secure(headers, asset, cycle, market_status, basket_level=None):
    ...
    try:
        ...
        print(
            f"{asset:10s} | V2437_ORDER_INTENT | "
            f"{level:<3} | {direction:4s} | {size:<10.6f} | "
            f"{market_status:9s} | level={payload.get('level')} stopDistance={payload.get('stopDistance')} guaranteedStop={payload.get('guaranteedStop')}"
        )
    except Exception as _v2437_e:
        print(f"{asset:10s} | V2437_ORDER_INTENT_ERROR | {level:<3} | {direction:4s} | err={_v2437_e}")
    ...
    try:
        ...
        print(
            f"{asset:10s} | V2437_ORDER_RESPONSE | "
            f"{level:<3} | {direction:4s} | {size:<10.6f} | "
            f"{market_status:9s} | HTTP={status} dealRef={short_id(deal_ref) if deal_ref else None}"
        )
    except Exception as _v2437_e:
        print(f"{asset:10s} | V2437_ORDER_RESPONSE_ERROR | {level:<3} | {direction:4s} | err={_v2437_e}")
    ...
    if status not in (200, 201, 202) or not deal_ref:
        ...
        try:
            ...
            print(
                f"{asset:10s} | V2437_ORDER_REJECTED | "
                f"{level:<3} | {direction:4s} | {size:<10.6f} | "
                f"{market_status:9s} | reason={reason}"
            )
        except Exception as _v2437_e:
            print(f"{asset:10s} | V2437_ORDER_REJECTED_ERROR | {level:<3} | {direction:4s} | err={_v2437_e}")
    ...
    try:
        event({
            "event": "V2437_ORDER_ACCEPTED",
            **record,
            "response": data,
        })
        print(
            f"{asset:10s} | V2437_ORDER_ACCEPTED | "
            f"{level:<3} | {direction:4s} | {size:<10.6f} | "
            f"{market_status:9s} | ref={short_id(deal_ref)} limit={payload.get('level')}"
        )
    except Exception as _v2437_e:
        print(f"{asset:10s} | V2437_ORDER_ACCEPTED_ERROR | {level:<3} | {direction:4s} | err={_v2437_e}")
...
def v24_open_basket_limits_secure(headers, asset, cycle, market_status):
    ...
    # V24.4 — BLOCAGE URGENCE DE L'ANCIENNE LOGIQUE BASKET LIMIT
    # Objectif : empêcher les anciens ordres L1/L2/L3 Bollinger en mode expérimental.
    try:
        import os
        if os.getenv("V244_DISABLE_CLASSIC_BASKET_OPEN", "1") == "1":
            ...
            print(
                f"{asset:10s} | V244_BLOCK_OPEN | ancien panier LIMIT L1/L2/L3 bloqué en mode expérimental"
            )

            return False, {
                "reason": "V244_CLASSIC_BASKET_OPEN_DISABLED",
                "asset": asset,
                "mode": "V24.4_EXPERIMENTAL_AUDIT"
            }
    except Exception as e:
        try:
            event({
...
def v24_close_basket_secure(headers, asset, basket_exec, reason):
    ...
    # Règle : si possible, fermer seulement une paire valide :
    # weak + strong, avec strong >= weak * 1.67.
    try:
