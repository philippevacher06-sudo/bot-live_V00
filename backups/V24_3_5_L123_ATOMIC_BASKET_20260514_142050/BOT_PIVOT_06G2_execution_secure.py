...
def v24_validate_bollinger_limit_levels(asset, direction, levels, price_audit=None, cycle_id=None):
    ...
    try:
        if price_audit and price_audit.get("snapshot_bid") is not None and price_audit.get("snapshot_ask") is not None:
            raw_bid = float(price_audit["snapshot_bid"])
            raw_ask = float(price_audit["snapshot_ask"])
        else:
            raw_bid, raw_ask = v24_market_bid_ask(asset)
    except Exception as exc:
        return {
...
def v24_enrich_cycle_with_bollinger_m5(headers, cycle):
    ...
    try:
        ...
        print(
            ...
            f"1-3 | {direction:4s} | --         | FILTER    | err={_e} allow_by_safety=True"
        )
