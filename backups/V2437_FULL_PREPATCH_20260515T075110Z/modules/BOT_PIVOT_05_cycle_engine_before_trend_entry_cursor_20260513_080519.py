    ...
    for asset in assets:
        ...
        if status != "IN_POSITION" or not cycle:
            if decision in ("BUY", "SELL"):
                ...
                # V23.2 — VETO de phase à l'entrée
                # BUY interdit contre TREND_DOWN
                # SELL interdit contre TREND_UP
                phase_veto_entry = (
                ...
                min_vwap_against = float(globals().get("MIN_CONTEXT_SCORE_ENTRY_VWAP_AGAINST", 4))
                ...
                if vwap_against and ctx_score < min_vwap_against:
                    if v24_relaxed_allow:
                        ...
                        ctx_score = max(float(ctx_score), float(MIN_CONTEXT_SCORE_ENTRY))
                    else:
                        print(
